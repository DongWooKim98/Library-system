		...
		<div id="section_wrap">
			...
			<div class="modify_account_form">
				...
				<form action="<c:url value='/user/member/modifyAccountConfirm' />" name="modify_account_form" method="post">
					...
					<input type="text" name="u_m_name" value="${loginedUserMemberVo.u_m_name}" placeholder="사용자 이름을 입력하세요."> <br>
					<select name="u_m_gender">
						<option value="">사용자 성별을 입력하세요.</option>
						<option value="M" <c:if test="${loginedUserMemberVo.u_m_gender eq 'M'}"> selected </c:if>>남자</option>
						<option value="W" <c:if test="${loginedUserMemberVo.u_m_gender eq 'W'}"> selected </c:if>>여자</option>
						
					</select> <br>
					<input type="email"	name="u_m_mail" value="${loginedUserMemberVo.u_m_mail}" placeholder="사용자 메일 주소를 입력하세요." ><br>
					<input type="text" name="u_m_phone"	value="${loginedUserMemberVo.u_m_phone}" placeholder="사용자 휴대폰 번호를 입력하세요."> <br>
